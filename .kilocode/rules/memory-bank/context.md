...
- [x] Added browser localStorage fallback for `/skill-roadmap` progress so task completion, JSON import/export, and GitHub backup still work on read-only deploy environments
...
| Date | Activity |
|------|----------|
| 2026-06-14 | Added browser localStorage fallback for roadmap progress on read-only deploy environments |
