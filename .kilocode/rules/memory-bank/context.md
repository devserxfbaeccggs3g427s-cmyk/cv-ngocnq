...
- [x] Expanded the Markdown book reader into a shared Markdown preview capability. Large reading-oriented previews now expose an icon-only book entry point, including standalone Markdown files, roadmap note previews, review task notes, comment/AI Markdown, image-analysis output, and quiz questions/explanations. Generic preview content is stored in browser storage and opened through `/markdown-reader/[readerId]`, while stored Markdown files still support `/markdown-files/read/[fileId]`. The reader is now a fixed full-screen overlay that covers Header/Footer/SideNav, uses a paper-only mobile surface with invisible left/right tap zones, keeps swipe/keyboard navigation and remembered page progress, and preserves horizontal scrolling inside code/table frames.
- [x] Fixed Markdown book reader content loss by replacing heuristic text-slice pagination with real CSS column pagination over the full rendered Markdown DOM. The reader now measures rendered width/pages after layout and Mermaid hydration, keeps code/table/Mermaid frames from being split when possible, lets code/table/Mermaid frames scroll internally, no longer overlays tap zones on top of scrollable content, and auto-detects Mermaid diagrams in unlabeled or text/plain fenced code blocks so they render as diagrams instead of plain code.
- [x] Improved the Markdown book reader feel so page changes no longer look like horizontal scrolling. The column offset now changes instantly under a paper-turn overlay animation, with next/previous flip directions, stronger paper texture, subtle fiber/aging gradients, page-edge shadows, and a mobile paper-only surface that still hides all surrounding app chrome.
- [x] Smoothed the book page-turn interaction further by delaying the actual page-content swap until the middle of the flip animation, extending the animation to 720ms, and moving the paper palette toward older warm parchment with more subtle stains, fibers, edge shadows, and aged mobile paper background.
- [x] Refined the Markdown reader again toward an old printed-book feel: removed line/repeating textures that read like notebook grid paper, shifted the page to aged parchment gradients and mottled stains, extended the turn animation to 880ms with a later mid-flip content commit, blocked overlapping turns during animation, and made code/table/callout/detail frames blend into the paper instead of using bright white panels.
- [x] Pushed the book reader styling further toward antique printed books. The reader now uses a darker mottled parchment palette without notebook-like line textures, body text uses a Georgia/Times-style serif stack with Palatino-style headings, code gutters and line-number areas are sepia/parchment instead of white, and code/table/callout frames are more translucent so they sit inside the page material.
- [x] Reworked the Markdown reader page-turn animation from a decorative overlay into a two-sided flipping sheet. During a turn, the app renders the old page on the sheet front and the target page on the sheet back while the target page sits underneath, then rotates the sheet through 180 degrees with backface visibility and directional shadows for a more natural page-turn effect.
- [x] Adjusted the Markdown reader after mobile reading feedback: added a safe bottom area to the CSS-column page flow so lines are not clipped at the bottom of the page, and lightened the page-turn animation by removing duplicated full Markdown renders from the animated sheet. The turn now commits the target page mid-animation while the animated layer is a lightweight paper sheet, reducing jank on long code/Mermaid-heavy notes.
- [x] Optimized the study/review mobile experience for readability and low-vision use. Mobile typography now uses a larger base size, Markdown/quiz content has larger line-height and font sizing, flashcard cards/buttons/stats have bigger touch targets, quiz options and navigation use 48px mobile actions, the review mindmap uses larger nodes and a readable mobile fit zoom instead of shrinking the whole canvas, and the task preview sheet/comment composer now uses larger mobile text and controls while preserving desktop density.
- [x] Increased the mobile study UI further toward a ChatGPT-like reading feel. Mobile base typography is now larger and calmer, study Markdown/quiz text uses more generous line height, flashcard fronts/backs and quiz options are larger, primary study actions use taller 56px targets on phones, mindmap fit zoom/node sizing is more readable, and focused AI/comment bubbles now use larger mobile text so long explanations read consistently.
...
- [x] Hardened the refreshed UI for light/dark consistency and mobile responsiveness. The app now declares color-scheme behavior, reduces mobile background/blur weight, adds responsive task-row indentation, tightens mobile container/section/card spacing, converts the mobile header menu into a viewport-bounded two-column drawer, scales the profile hero/contact/CTA layout for phones, keeps roadmap task status beside content on mobile, improves Markdown workspace controls/editor height for small screens, switches AI Context to a professional two-column layout from `lg` upward, and caps source/history panel heights on mobile so chat/results remain reachable.
- [x] Refreshed the main app visual design into a more modern premium/glass interface. Global CSS now defines a richer blue/violet/cyan palette, gradient page background, subtle grid overlay, reusable glass/input/card utilities, and updated shared UI primitives for buttons, badges, cards, containers, sections, modals, comment forms, and AI provider settings. The home CV hero, summary, experience timeline, skills, education, certifications, languages, portfolio listing/cards, contact section/form, header, side nav, footer, roadmap hero/filter/track/task/backup panels, Markdown workspace header/sidebar, AI Context workspace, and AI Image Analysis workspace were polished while preserving existing data flow and feature behavior.
- [x] Clarified the `/ai-image-analysis` workflow for starting a separate analysis/chat. The page now has explicit "Phân tích mới" actions in the header and history rail, resets uploaded images/prompt/comment draft/thread UI for a fresh analysis, clears temporary uploads when restoring an old history item, and automatically switches to a new-analysis state when users select/upload images while viewing history. The image metric now reflects the selected history result's saved image count when a history item is active.
- [x] Extended `/ai-image-analysis` with persisted analysis history and result comments. Each successful image analysis is saved as a root `StudyComment` under `skill-roadmap-study-comments:v1` with a new `image-analysis` context carrying analysis id, focus type, prompt, image count/names, provider, and model metadata. The workspace now shows a history rail, restores old results, deletes full analysis threads, auto-generates best-effort history titles through `/api/ai/context-title`, and lets users comment or ask AI follow-up questions against the selected analysis result. Because it reuses `studyComments`, export/import/GitHub backup keeps the image-analysis history and comments without changing the backup payload version.
- [x] Added `/ai-image-analysis`, an independent image-analysis workspace where users can upload up to 4 PNG/JPG/WebP images, choose a focused analysis type such as Chứng khoán, Kỹ thuật, Bảng dữ liệu, Tài liệu, or Tổng quát, enter a custom prompt, configure the existing OpenAI-compatible provider/model flow, and render the AI result through the shared Markdown preview. The new `/api/ai/image-analysis` route sends multimodal chat-completion payloads with image data URLs, reuses shared provider/API-key helpers, supports server-side Kilo credentials with `AI_ENV_CONFIRM_PASSWORD`, and can use `AI_IMAGE_ANALYSIS_MODEL` as the default vision model.
- [x] Simplified the `/skill-roadmap` task list for a fully completed review flow: inline leaf-task AI prompt cards and completed-task note textareas were removed from roadmap rows, leaving the list focused on hierarchy, status, metadata, deliverables, and completion time. Full prompt/note workflows remain available from task detail, note, and study screens.
...
- `/workspace` is now the central hub for feature-heavy workflows: Skill Roadmap, Markdown Files, AI Context, AI Image Analysis, and Workspace Backup. Public navigation stays focused on the portfolio/CV, while Header, mobile menu, SideNav, Footer, and homepage preview route users into Workspace for tools.
...
- Task detail pages also show the generated AI learning prompt for the current task with show/hide and copy controls, using the same theory-focused prompt structure previously used in roadmap rows
...
- `/ai-image-analysis` provides a standalone AI vision workspace for screenshots, charts, tables, and technical images. Users upload images in-browser, pick a focus category, enter a prompt, and call `/api/ai/image-analysis` through the same OpenAI-compatible provider settings used elsewhere. Completed analyses are stored in `skill-roadmap-study-comments:v1` using the `image-analysis` context, show up in a restorable/deletable history rail with AI-generated titles, and support normal comments plus AI follow-up replies scoped to the selected result. Kilo/server-side usage requires `AI_ENV_CONFIRM_PASSWORD`; `AI_IMAGE_ANALYSIS_MODEL` can provide the default vision model without exposing API credentials.
...
- Roadmap task rows now stay compact by hiding inline AI prompt cards and completed-task note textareas/links in the tree view; task detail, note, flashcard, and quiz screens remain the places for full prompt/note workflows.
...
- Each roadmap node can still generate a concise AI learning prompt for detail/AI workflows; prompts emphasize theory, internal mechanisms, why/how explanations, trade-offs, and deep interview questions
...
- `/workspace/backup` contains the backup tools for workspace data: browser JSON export/import, optional GitHub commit backup, and reset/reseed for local browser data. GitHub tokens are submitted per request and not persisted by the app.
- Workspace Backup includes a confirmed local reset action that removes browser workspace storage and reloads the newest seed from `src/data/skill-roadmap-progress.json` through `/api/skill-roadmap/progress`.
...
- Completed task note inputs live on task detail screens rather than the `/skill-roadmap` list, keeping the review list focused on scan-friendly task status.
...
- `/workspace/backup` exports a versioned JSON object containing `progress`, `comments`, `flashcards`, `quizzes`, `studyComments`, and `markdownFiles`, where comments come from `skill-roadmap-note-comments:v1`, flashcards come from `skill-roadmap-flashcards:v1` as `Record<taskId, FlashcardDeck[]>`, quizzes come from `skill-roadmap-quizzes:v1`, flashcard/quiz/AI Context/image-analysis threads come from `skill-roadmap-study-comments:v1`, and standalone Markdown files come from `markdown-files:v1`. Import accepts the current combined format, older single-flashcard-deck backups, older comment/flashcard/quiz backups without `studyComments`/`markdownFiles`, and older progress-only JSON backups. GitHub backup commits the combined payload, and the Reset Workspace action now clears progress, comment, flashcard, quiz, study-comment, and markdown-file storage before reloading from the project seed.
...
| 2026-07-12 | Hardened the refreshed UI for light/dark behavior and responsive/mobile UX, including mobile header drawer, profile hero scaling, roadmap task indentation, Markdown workspace height controls, and AI workspace panel sizing |
| 2026-07-12 | Refreshed the app UI with a premium glass/gradient visual system across shared primitives, CV/home sections, portfolio, roadmap, Markdown, AI Context, AI Image Analysis, contact, header/nav, and footer |
