...
- `/skill-roadmap` backup now exports a versioned JSON object containing `progress`, `comments`, and `flashcards`, where comments come from `skill-roadmap-note-comments:v1` and flashcards come from `skill-roadmap-flashcards:v1`. Import accepts the current combined format, older comment-only combined backups, and older progress-only JSON backups. GitHub backup commits the combined payload, and the Clear localStorage action now clears progress, comment, and flashcard storage before reloading progress from the project seed.
