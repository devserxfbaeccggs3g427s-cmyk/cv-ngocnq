...
export function SkillRoadmapTaskDetail({ task }: { task: TaskContext }) {
  ...
  useEffect(() => {
    ...
    async function hydrateMissingSeedData() {
      if (hasLocalProgress && hasLocalComments) {
        return;
      }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-700 transition hover:border-violet-400 hover:text-violet-800 dark:border-violet-900/70 dark:text-violet-300 dark:hover:border-violet-700 dark:hover:text-violet-200"
          >
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <h2 className="mt-1 flex items-center gap-2 text-lg font-bold text-gray-950 dark:text-white">
                    ...
                    Ôn tập trên màn riêng
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
                    Flashcard dùng toàn bộ note và {noteComments.length} comment của task này, hỗ trợ tạo nhiều bộ thẻ theo các góc học khác nhau.
                  </p>
                </div>
                <Link
                  href={`/skill-roadmap/tasks/${encodeURIComponent(task.id)}/flashcards`}
                  className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-semibold text-white transition hover:bg-violet-700"
                >
                  <Brain className="h-4 w-4" />
                  Mở flashcard
                </Link>
